...
public class PayServiceImpl implements PayService,DlbUrlConfig {
    ...
    @Override
    public String CommUrlFunV2(String urlType, JSONObject jsonParam) {
        String response="";
        log.info(urlType+" "+" 获取的参数: {}",jsonParam.toJSONString());
        Request request=null;
        try{
        ...
        switch (urlType){
            case payOrder:
                //TODO 支付订单
                try{
                    //TODO 这里还是要浅度克隆  方法虽然上了锁  但是方法里面的对象的引用还是会变(并发时会变)  所以需要克隆
                    Request requestV2=(Request)request.clone();
                    response=CommonServiceImpl.payOrder(requestV2, dlpPayConfigEntityMapper,
                            dlbConnfig, restTemplate,orderMoneyLogMapper);
                }catch (Exception e){
                    e.printStackTrace();
                    log.error("pay 下单失败");
                    response=ResponseDlb(request,ErrorEnum.SSCO003000,null);
            ...
            case queryPayOrder:
                ...
                try{
                    ...
                    if(dlpPayConfigEntity==null){
                        log.info("dlpPayConfigEntity {}","查询出来的dlb支付配置为空");
                        log.error("dlpPayConfigEntity {}","查询出来的dlb支付配置为空");
                        return ResponseDlb(request,ErrorEnum.SSCO003000,null);
                    }
                    ...
                    String requestNum=request.getTradeNo();
                    String urlAfter="/v1/agent/order/payresult/"
                            +dlpPayConfigEntity.getAgentnum()+"/"
                            +dlpPayConfigEntity.getCustomernum()+"/"
                            +dlpPayConfigEntity.getShopnum()+"/"+requestNum;
                    String url = "https://openapi.duolabao.com"+urlAfter;
                    HttpHeaders headers = new HttpHeaders();
                    ...
                    headers.set("token",sign);
                    MultiValueMap<String, String> requestBody = new LinkedMultiValueMap<>();
                    requestBody.add("agentNum", dlpPayConfigEntity.getAgentnum());
                    requestBody.add("customerNum", dlpPayConfigEntity.getCustomernum());
                    requestBody.add("shopNum", dlpPayConfigEntity.getShopnum());
                    requestBody.add("requestNum", requestNum);
                    HttpEntity<MultiValueMap> requestEntity = new HttpEntity<MultiValueMap>(requestBody, headers);
                    ResponseEntity<String> responseEntity =responseEntity = restTemplate.exchange(
                            url,
